...
use crate::{BSplineBasis, KnotVector, RealKnotVector, ComplexKnotVector};
...
impl<'a> BSplineBasisEvaluator<'a, ComplexKnotVector> {
    pub fn b(&self, i: usize, x: f64) -> <ComplexKnotVector as KnotVector>::Scalar {
        crate::core::evaluation::b(
            i,
            crate::core::ecs::ecs_x(x, self.basis.knot_vector.config.r0, self.basis.knot_vector.config.eta),
            self.basis.knot_vector(),
            self.basis.degree(),
        )
    }
